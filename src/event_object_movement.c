...
#ifdef PORTABLE
static inline bool8 IsValidObjectEvent(struct ObjectEvent *obj)
{
    return obj >= &gObjectEvents[0] && obj < &gObjectEvents[OBJECT_EVENTS_COUNT];
}
static inline bool8 IsValidSprite(struct Sprite *spr)
{
    return spr >= &gSprites[0] && spr < &gSprites[MAX_SPRITES];
}
#define SAFE_GET_OBJECT_EVENT(sprite) \
    (IsValidSprite(sprite) && (u32)(sprite)->data[0] < OBJECT_EVENTS_COUNT ? &gObjectEvents[(sprite)->data[0]] : NULL)
#else
#define SAFE_GET_OBJECT_EVENT(sprite) (&gObjectEvents[(sprite)->data[0]])
#endif
...
{                                                                                                \
    struct ObjectEvent *obj = SAFE_GET_OBJECT_EVENT(sprite);                                     \
    if (obj) UpdateObjectEventCurrentMovement(obj, sprite, setup##_callback);                    \
}                                                                                                \
...
#define OBJ_EVENT_PAL_TAG_RS_BRENDAN                  0x111C
#define OBJ_EVENT_PAL_TAG_RS_MAY                      0x111D
...
static const struct SpritePalette sObjectEventSpritePalettes[] = {
    ...
    {gObjectEventPal_RSBrendan,               OBJ_EVENT_PAL_TAG_RS_BRENDAN},
    {gObjectEventPal_RSMay,                   OBJ_EVENT_PAL_TAG_RS_MAY},
    {},
};
...
#ifdef PORTABLE
                && !(objectEvent->localId >= 240 && objectEvent->localId < 248)
#endif
            )
...
const struct ObjectEventGraphicsInfo *GetObjectEventGraphicsInfo(u8 graphicsId)
{
...
#ifdef PORTABLE
    {
        ...
        extern bool8 AssetLoader_LoadOverworldSprite(u16 gfxId, void *dest, int maxSize);
        if (AssetLoader_IsActive())
        {
            #define MAX_OW_OVERRIDES 64
            static u8 sOwSpriteBufs[MAX_OW_OVERRIDES][0x2000];
            static struct SpriteFrameImage sOwFrameImages[MAX_OW_OVERRIDES];
            static struct ObjectEventGraphicsInfo sOwInfos[MAX_OW_OVERRIDES];
            static u8 sOwGfxIds[MAX_OW_OVERRIDES];
            static int sOwCount = 0;
            static bool8 sOwInited = FALSE;

            if (!sOwInited) { memset(sOwGfxIds, 0xFF, sizeof(sOwGfxIds)); sOwInited = TRUE; }

            // Check if already loaded
            int slot = -1;
            for (int i = 0; i < sOwCount; i++) {
                if (sOwGfxIds[i] == graphicsId) { slot = i; break; }
            }

            if (slot < 0 && sOwCount < MAX_OW_OVERRIDES) {
                if (AssetLoader_LoadOverworldSprite(graphicsId, sOwSpriteBufs[sOwCount], sizeof(sOwSpriteBufs[0]))) {
                    slot = sOwCount;
                    sOwGfxIds[slot] = graphicsId;
                    sOwInfos[slot] = *gObjectEventGraphicsInfoPointers[graphicsId];
                    sOwFrameImages[slot].data = sOwSpriteBufs[slot];
                    sOwFrameImages[slot].size = sOwInfos[slot].size;
                    sOwInfos[slot].images = &sOwFrameImages[slot];
                    sOwCount++;
                }
            }

            if (slot >= 0)
                return &sOwInfos[slot];
        }
    }
#endif
...
void __attribute__((noinline,optimize("O0"))) UpdateObjectEventCurrentMovement(struct ObjectEvent *objectEvent, struct Sprite *sprite, bool8 (*callback)(struct ObjectEvent *, struct Sprite *))
{
#ifdef PORTABLE
    if (!sprite || !objectEvent || !IsValidObjectEvent(objectEvent) || !IsValidSprite(sprite) || !objectEvent->active)
        return;
#endif
...
{
#ifdef PORTABLE
    if (!sprite || !objectEvent || !IsValidObjectEvent(objectEvent) || !objectEvent->active)
        return;
#endif
    DoGroundEffects_OnSpawn(objectEvent, sprite);
    TryEnableObjectEventAnim(objectEvent, sprite);
...
{
#ifdef PORTABLE
    if (!sprite || !objectEvent || !IsValidObjectEvent(objectEvent))
        return;
#endif
...
#ifdef PORTABLE
    if (!objectEvent || !IsValidObjectEvent(objectEvent))
        return;
#endif
...
static void __attribute__((noinline,optimize("O0"))) GetGroundEffectFlags_ShallowFlowingWater(struct ObjectEvent *objEvent, u32 *flags)
{
...
#ifdef PORTABLE
static volatile int sGroundEffectsDisabled = 0;
void SetGroundEffectsDisabled(int disabled) { sGroundEffectsDisabled = disabled; }
#endif

static void __attribute__((noinline,optimize("O0"))) DoGroundEffects_OnSpawn(struct ObjectEvent *objEvent, struct Sprite *sprite)
{
    u32 flags;

#ifdef PORTABLE
    if (sGroundEffectsDisabled || !IsValidObjectEvent(objEvent)) return;
#endif
...
static void __attribute__((noinline,optimize("O0"))) DoGroundEffects_OnBeginStep(struct ObjectEvent *objEvent, struct Sprite *sprite)
{
    u32 flags;
...
static void __attribute__((noinline,optimize("O0"))) DoGroundEffects_OnFinishStep(struct ObjectEvent *objEvent, struct Sprite *sprite)
{
    u32 flags;
