...
__attribute__((dllimport)) int __stdcall IsBadReadPtr(const void *lp, unsigned int ucb);
#endif
#include "gflib.h"
#include "overworld.h"
#include "script.h"
...
#include "quest_log.h"
#include "fieldmap.h"
#ifdef PORTABLE_TILESET_OVERRIDE
#include <stdio.h>
#ifdef _WIN32
#include <direct.h>
...
#endif
#include "platform/tileset_registry.h"
static const char *FindTilesetPath(const struct Tileset *tileset);
#endif
...
static void InitMapLayoutData(struct MapHeader * mapHeader)
{
...
#ifdef PORTABLE_TILESET_OVERRIDE
    {
        // Try loading metatile overrides for both tilesets
        extern bool8 AssetLoader_IsActive(void);
        extern int AssetLoader_Load(const char *relativePath, void *dest, int maxSize);
        if (AssetLoader_IsActive())
        {
            static u16 sMetatileOverridePrimary[0x4000];
            static u32 sMetaAttrOverridePrimary[0x2000];
            static u16 sMetatileOverrideSecondary[0x4000];
            static u32 sMetaAttrOverrideSecondary[0x2000];

            if (mapLayout->primaryTileset)
            {
                const char *tsPath = FindTilesetPath(mapLayout->primaryTileset);
                if (tsPath)
                {
                    char path[256];
                    snprintf(path, sizeof(path), "%s/metatiles.bin", tsPath);
                    if (AssetLoader_Load(path, sMetatileOverridePrimary, sizeof(sMetatileOverridePrimary)) > 0)
                        ((struct Tileset *)mapLayout->primaryTileset)->metatiles = sMetatileOverridePrimary;

                    snprintf(path, sizeof(path), "%s/metatile_attributes.bin", tsPath);
                    if (AssetLoader_Load(path, sMetaAttrOverridePrimary, sizeof(sMetaAttrOverridePrimary)) > 0)
                        ((struct Tileset *)mapLayout->primaryTileset)->metatileAttributes = sMetaAttrOverridePrimary;
                }
            }
            if (mapLayout->secondaryTileset)
            {
                const char *tsPath = FindTilesetPath(mapLayout->secondaryTileset);
                if (tsPath)
                {
                    char path[256];
                    snprintf(path, sizeof(path), "%s/metatiles.bin", tsPath);
                    if (AssetLoader_Load(path, sMetatileOverrideSecondary, sizeof(sMetatileOverrideSecondary)) > 0)
                        ((struct Tileset *)mapLayout->secondaryTileset)->metatiles = sMetatileOverrideSecondary;

                    snprintf(path, sizeof(path), "%s/metatile_attributes.bin", tsPath);
                    if (AssetLoader_Load(path, sMetaAttrOverrideSecondary, sizeof(sMetaAttrOverrideSecondary)) > 0)
                        ((struct Tileset *)mapLayout->secondaryTileset)->metatileAttributes = sMetaAttrOverrideSecondary;
                }
            }
        }
    }
    ...
    {
        extern bool8 AssetLoader_IsActive(void);
        extern int AssetLoader_Load(const char *path, void *dest, int maxSize);
        if (AssetLoader_IsActive())
        {
            static u16 sMapGridOverride[0x8000];
            char mpath[256];
            extern struct SaveBlock1 *gSaveBlock1Ptr;
            snprintf(mpath, sizeof(mpath), "maps/%d_%d/map.bin",
                gSaveBlock1Ptr->location.mapGroup, gSaveBlock1Ptr->location.mapNum);
            int gridSize = mapLayout->width * mapLayout->height * 2;
            if (AssetLoader_Load(mpath, sMapGridOverride, gridSize) > 0)
            {
                InitBackupMapLayoutData(sMapGridOverride, mapLayout->width, mapLayout->height);
...
static u32 GetAttributeByMetatileIdAndMapLayout(const struct MapLayout *mapLayout, u16 metatile, u8 attributeType)
{
    ...
    u16 primaryCount = NUM_METATILES_IN_PRIMARY;
#if defined(PORTABLE) && defined(_WIN32)
    {
        if (!mapLayout || IsBadReadPtr(mapLayout, sizeof(*mapLayout)))
            return 0xFF;
        if (!mapLayout->primaryTileset || IsBadReadPtr(mapLayout->primaryTileset, sizeof(struct Tileset)))
            return 0xFF;
        if (!mapLayout->secondaryTileset || IsBadReadPtr(mapLayout->secondaryTileset, sizeof(struct Tileset)))
            return 0xFF;
    }
#endif
    if (metatile < primaryCount)
    {
        attributes = mapLayout->primaryTileset->metatileAttributes;
#if defined(PORTABLE) && defined(_WIN32)
        if (!attributes || IsBadReadPtr(attributes, (metatile + 1) * sizeof(u32)))
            return 0xFF;
#endif
...
#if defined(PORTABLE) && defined(_WIN32)
        if (!attributes || IsBadReadPtr(attributes, (metatile - primaryCount + 1) * sizeof(u32)))
            return 0xFF;
#endif
        return ExtractMetatileAttribute(attributes[metatile - primaryCount], attributeType);
    }
    else
    {
        return 0xFF;
...
static const char *FindTilesetPath(const struct Tileset *tileset)
{
    for (int i = 0; i < NUM_TILESET_PATHS; i++)
        if (sTilesetPaths[i].tileset == tileset)
            return sTilesetPaths[i].path;
    return NULL;
}

// Dump a tileset's data to extracted_assets/ if not already present
static void DumpTilesetIfNeeded(const struct Tileset *tileset)
{
    const char *tsPath = FindTilesetPath(tileset);
    if (!tsPath) return;
    ...
    char checkPath[256];
    snprintf(checkPath, sizeof(checkPath), "%s/tiles.4bpp", tsPath);
    u8 testByte;
    if (AssetLoader_Load(checkPath, &testByte, 1) > 0) return; // already dumped

    // Create directory and dump
    char dirPath[256];
    snprintf(dirPath, sizeof(dirPath), "extracted_assets/frlg/%s", tsPath);
    {
        char tmp[256];
    ...
    // Decompress and write tiles
    if (tileset->tiles && tileset->isCompressed) {
        static u8 sTileDumpBuf[0x10000];
        extern void LZ77UnCompWram(const void *src, void *dst);
        LZ77UnCompWram(tileset->tiles, sTileDumpBuf);
        u32 decompSize = ((const u8 *)tileset->tiles)[1] | (((const u8 *)tileset->tiles)[2]<<8) | (((const u8 *)tileset->tiles)[3]<<16);
        char filePath[256];
        snprintf(filePath, sizeof(filePath), "extracted_assets/frlg/%s/tiles.4bpp", tsPath);
        FILE *f = fopen(filePath, "wb");
        if (f) { fwrite(sTileDumpBuf, 1, decompSize, f); fclose(f); }
    }

    // Write metatiles
    if (tileset->metatiles) {
        // Metatile count depends on primary/secondary
        int numMetatiles = tileset->isSecondary ? 640 : 640;
        int metatileDataSize = numMetatiles * 16; // each metatile is 8 u16 values = 16 bytes
        char filePath[256];
        snprintf(filePath, sizeof(filePath), "extracted_assets/frlg/%s/metatiles.bin", tsPath);
        FILE *f = fopen(filePath, "wb");
        if (f) { fwrite(tileset->metatiles, 1, metatileDataSize, f); fclose(f); }
    }

    // Write metatile attributes
    if (tileset->metatileAttributes) {
        int numMetatiles = 640;
        char filePath[256];
        snprintf(filePath, sizeof(filePath), "extracted_assets/frlg/%s/metatile_attributes.bin", tsPath);
        FILE *f = fopen(filePath, "wb");
        if (f) { fwrite(tileset->metatileAttributes, 4, numMetatiles, f); fclose(f); }
    }

    // Write palettes
    if (tileset->palettes) {
        int startPal = tileset->isSecondary ? 6 : 0;
        int numPals = tileset->isSecondary ? 7 : 6;
        char palDir[256];
        snprintf(palDir, sizeof(palDir), "extracted_assets/frlg/%s/palettes", tsPath);
#ifdef _WIN32
        MKDIR(palDir);
#else
        mkdir(palDir, 0755);
        ...
        for (int p = 0; p < numPals; p++) {
            char filePath[256];
            snprintf(filePath, sizeof(filePath), "extracted_assets/frlg/%s/palettes/%02d.gbapal", tsPath, startPal + p);
            FILE *f = fopen(filePath, "wb");
            if (f) { fwrite(tileset->palettes[startPal + p], 1, 32, f); fclose(f); }
        }
    }
}
#endif
...
static void CopyTilesetToVram(struct Tileset const *tileset, u16 numTiles, u16 offset)
{
    if (tileset)
    {
#ifdef PORTABLE_TILESET_OVERRIDE
        {
            ...
            if (AssetLoader_IsActive())
            {
                DumpTilesetIfNeeded(tileset);
                const char *tsPath = FindTilesetPath(tileset);
                if (tsPath)
                {
                    static u8 sTilesBuf[0x10000];
                    char tpath[256];
                    snprintf(tpath, sizeof(tpath), "%s/tiles.4bpp", tsPath);
                    int size = AssetLoader_Load(tpath, sTilesBuf, sizeof(sTilesBuf));
                    if (size > 0)
                    {
                        LoadBgTiles(2, sTilesBuf, numTiles * 32, offset);
                        return;
                    }
                }
            }
    ...
    if (tileset)
    {
#ifdef PORTABLE_TILESET_OVERRIDE
        {
            ...
            if (AssetLoader_IsActive())
            {
                ...
                if (tsPath)
                {
                    static u8 sTilesBuf2[0x10000];
                    char path[256];
                    snprintf(path, sizeof(path), "%s/tiles.4bpp", tsPath);
                    int size = AssetLoader_Load(path, sTilesBuf2, sizeof(sTilesBuf2));
                    if (size > 0)
                    {
                        LoadBgTiles(2, sTilesBuf2, numTiles * 32, offset);
                        return;
                    }
                }
            }
    ...
    if (tileset)
    {
#ifdef PORTABLE_TILESET_OVERRIDE
        {
            ...
            if (AssetLoader_IsActive())
            {
                ...
                if (tsPath)
                {
                    // Try loading palette files from the tileset's palettes/ subdirectory
                    static u16 sPalBuf[256];
                    char path[256];
                    int startPal = (tileset->isSecondary == TRUE) ? NUM_PALS_IN_PRIMARY : 0;
                    int numPals = size / PLTT_SIZE_4BPP;
                    int loaded = 0;

                    for (int p = 0; p < numPals; p++)
                    {
                        snprintf(path, sizeof(path), "%s/palettes/%02d.gbapal", tsPath, startPal + p);
                        int sz = AssetLoader_Load(path, &sPalBuf[p * 16], 32);
                        if (sz > 0) loaded++;
                    }
                    ...
                    if (loaded > 0)
                    {
                        if (tileset->isSecondary == FALSE)
                        {
                            ...
                            LoadPalette(sPalBuf + 1, destOffset + 1, size - PLTT_SIZEOF(1));
                        }
                        else
                        {
                            LoadPalette(sPalBuf, destOffset, size);
                        }
