...
#ifdef PORTABLE
    extern bool8 NetLink_IsActive(void);
    extern u8 NetLink_GetMultiplayerId(void);
    if (NetLink_IsActive())
...
#ifdef PORTABLE
    extern bool8 NetLink_IsActive(void);
    extern bool8 NetLink_SendBlock(const void *src, u16 size);
    if (NetLink_IsActive())
        return NetLink_SendBlock(src, size);
#endif
    if (gWirelessCommType == 1)
...
bool8 SendBlockRequest(u8 blockRequestType)
{
    ...
    if (NetLink_IsActive())
    {
        SendBlock(0, gBlockSendBuffer, BLOCK_BUFFER_SIZE);
        extern void NetLink_RequestSelfSend(void);
        NetLink_RequestSelfSend();
        return TRUE;
    }
#endif
    if (gWirelessCommType == 1)
...
#ifdef PORTABLE
    extern bool8 NetLink_IsActive(void);
    extern u8 NetLink_GetBlockReceivedStatus(void);
    if (NetLink_IsActive())
        return NetLink_GetBlockReceivedStatus();
#endif
    if (gWirelessCommType == 1)
...
void ResetBlockReceivedFlags(void)
{
    ...
    if (NetLink_IsActive())
    {
        extern void NetLink_ResetBlockReceivedFlag(u8);
        for (i = 0; i < MAX_LINK_PLAYERS; i++)
            NetLink_ResetBlockReceivedFlag(i);
        return;
    }
#endif
    if (gWirelessCommType == 1)
...
#ifdef PORTABLE
    extern bool8 NetLink_IsActive(void);
    extern void NetLink_ResetBlockReceivedFlag(u8 playerId);
    if (NetLink_IsActive())
    {
        NetLink_ResetBlockReceivedFlag(who);
        return;
    }
#endif
    if (gWirelessCommType == 1)
