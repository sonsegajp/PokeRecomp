...
extern const u16 gTitleScreen_LG_Slash_Pal[];
#endif
...
#ifdef PORTABLE
#define TITLE_SPECIES (gIsLeafGreen ? SPECIES_VENUSAUR : SPECIES_CHARIZARD)
...
#endif
#if !defined(PORTABLE) && defined(LEAFGREEN)
#define sBorderBgMap sBorderBgMap_LG
#endif
...
static const union AnimCmd *const sSpriteAnim_FlameAnims[] = {
    sSpriteAnim_Flame,
...
static const union AnimCmd *const sSpriteAnim_LeafAnims[] = {
    sSpriteAnim_Leaf
};
#endif

#ifdef PORTABLE
#define sSpriteAnim_FlameOrLeaf (gIsLeafGreen ? sSpriteAnim_LeafAnims : sSpriteAnim_FlameAnims)
#elif defined(FIRERED)
#define sSpriteAnim_FlameOrLeaf sSpriteAnim_FlameAnims
#elif defined(LEAFGREEN)
#define sSpriteAnim_FlameOrLeaf sSpriteAnim_LeafAnims
#endif
...
#ifdef PORTABLE
#define sSpriteSheets (gIsLeafGreen ? sSpriteSheets_LG : sSpriteSheets_FR)
#define sSpritePals   (gIsLeafGreen ? sSpritePals_LG : sSpritePals_FR)
#elif defined(FIRERED)
#define sSpriteSheets sSpriteSheets_FR
#define sSpritePals   sSpritePals_FR
#elif defined(LEAFGREEN)
#define sSpriteSheets sSpriteSheets_LG
#define sSpritePals   sSpritePals_LG
#endif
...
#ifdef PORTABLE
        LoadPalette(gIsLeafGreen ? gGraphics_TitleScreen_LG_BackgroundPals : gGraphics_TitleScreen_BackgroundPals, BG_PLTT_ID(14), PLTT_SIZE_4BPP);
#else
...
#ifdef PORTABLE
        DecompressAndCopyTileDataToVram(3, gIsLeafGreen ? sBorderBgMap_LG : sBorderBgMap, 0, 0, 1);
#else
...
static void SetTitleScreenScene_FadeIn(s16 *data)
{
    ...
    case 8:
        if (!IsBlendPalettesGraduallyTaskActive(0))
        {
            ...
            ShowBg(0);
#ifdef PORTABLE
            CpuCopy16(gIsLeafGreen ? gGraphics_TitleScreen_LG_BoxArtMonPals : gGraphics_TitleScreen_BoxArtMonPals, &gPlttBufferUnfaded[BG_PLTT_ID(13)], PLTT_SIZE_4BPP);
#else
...
#ifdef PORTABLE
        CreateTask(gIsLeafGreen ? Task_LeafSpawner : Task_FlameSpawner, 5);
#elif defined(FIRERED)
...
static void LoadSpriteGfxAndPals(void)
{
    s32 i;
#ifdef PORTABLE
    const struct CompressedSpriteSheet *sheets = gIsLeafGreen ? sSpriteSheets_LG : sSpriteSheets_FR;
    const struct SpritePalette *pals = gIsLeafGreen ? sSpritePals_LG : sSpritePals_FR;
    for (i = 0; i < 4; i++)
        LoadCompressedSpriteSheet(&sheets[i]);
    LoadSpritePalettes(pals);
#else
