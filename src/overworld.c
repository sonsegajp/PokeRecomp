...
// Not static: src/platform/net_client.c calls this after receiving save data
// from the server. It was previously declared extern there but defined static
// here, so the reference never linked (the PC build tolerates unresolved
// symbols) and online login jumped to a bogus address.
void LoadSaveblockObjEventScripts(void)
{
    ...
    // Vanilla reads src[i] for all OBJECT_EVENT_TEMPLATES_COUNT (64) slots
    // regardless of how many object events the map actually has. On the GBA
    // that just read adjacent ROM and was harmless. Here the array is a real
    // allocation, so a map with few object events makes this read up to
    // ~1.5KB past the end — a segfault whenever it lands past a page boundary.
    // Clamp to the real count.
...
// Not static: see the note on LoadSaveblockObjEventScripts above — this is
// called from src/platform/net_client.c on online login.
...
#ifdef PORTABLE
static bool8 sOverworldActive = FALSE;
bool8 IsOverworldActive(void) { return sOverworldActive; }
void NetClient_ResetOverworldFlag(void) { sOverworldActive = FALSE; }
#endif

static void OverworldBasic(void)
{
#ifdef PORTABLE
    sOverworldActive = TRUE;
#endif
...
#ifdef PORTABLE
    {
        extern void NetPlayerSprites_Update(void);
        ...
        if (NetClient_IsConnected())
        {
            ...
            if (++sSendCounter >= 4)
            {
                ...
                u8 running = (gPlayerAvatar.flags & PLAYER_AVATAR_FLAG_DASH) ? 1 : 0;
                extern bool8 ScriptContext_IsEnabled(void);
                extern bool8 ArePlayerFieldControlsLocked(void);
                u8 busy = (ScriptContext_IsEnabled() || ArePlayerFieldControlsLocked()) ? 1 : 0;
                u8 status = running | (busy << 1);
...
void CB2_ContinueSavedGame(void)
{
    ...
    printf("[CB2_ContinueSavedGame] ENTERED sb1=%p sb2=%p storage=%p\n",
           (void*)gSaveBlock1Ptr, (void*)gSaveBlock2Ptr, (void*)gPokemonStoragePtr);
    fflush(stdout);
    if (gSaveBlock1Ptr == NULL) gSaveBlock1Ptr = &gSaveBlock1;
    if (gSaveBlock2Ptr == NULL) gSaveBlock2Ptr = &gSaveBlock2;
    if (gPokemonStoragePtr == NULL) gPokemonStoragePtr = &gPokemonStorage;
#endif
    FieldClearVBlankHBlankCallbacks();
#ifdef PORTABLE
    ResetTasks();
    ...
    ResetSafariZoneFlag_();
#ifdef PORTABLE
    printf("[CB2_Continue] LoadSaveblockMapHeader...\n"); fflush(stdout);
#endif
    LoadSaveblockMapHeader();
...
#ifdef PORTABLE
    printf("[CB2_Continue] InitMapFromSavedGame...\n"); fflush(stdout);
#endif
...
#ifdef PORTABLE
    printf("[CB2_Continue] UseContinueGameWarp=%d\n", UseContinueGameWarp()); fflush(stdout);
#endif
    ...
    else
    {
        ...
        SetMainCallback1(CB1_Overworld);
#ifdef PORTABLE
        printf("[CB2_Continue] CB2_ReturnToField...\n"); fflush(stdout);
#endif
        CB2_ReturnToField();
#ifdef PORTABLE
        printf("[CB2_Continue] done\n"); fflush(stdout);
#endif
    }
}
...
#ifdef PORTABLE
        {
            extern void SetGroundEffectsDisabled(int);
            SetGroundEffectsDisabled(0);
            extern void NetPlayerSprites_OnMapLoad(void);
            NetPlayerSprites_OnMapLoad();
        }
#endif
        return TRUE;
    }
