...
static bool8 HandleMenuInput(u8 taskId)
{
    if (JOY_NEW(A_BUTTON))
    {
        ...
        {
            extern void ReloadSave(void);
            printf("[HandleMenuInput] A pressed! menuType=%d cursorPos=%d\n",
                   gTasks[taskId].tMenuType, gTasks[taskId].tCursorPos);
            fflush(stdout);
            if (gTasks[taskId].tMenuType >= MAIN_MENU_CONTINUE && gTasks[taskId].tCursorPos == 0)
            {
                PlaySE(SE_SELECT);
                FreeAllWindowBuffers();
                DestroyTask(taskId);
