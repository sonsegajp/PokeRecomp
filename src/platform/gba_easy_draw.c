...
void SetWidescreenEnabled(int enabled) { (void)enabled; }
void SetDesiredRenderWidth(int w) { (void)w; }
int GetRenderWidth(void) { return DISPLAY_WIDTH; }
...
static void DrawSprites(struct scanlineData* scanline, uint16_t vcount, bool windowsEnabled)
{
    ...
    for (i = 127; i >= 0; i--)
    {
        ...
        if (isAffine)
        {
            ...
            matrix[0][0] = gOamMatrices[matrixNum].a;
            matrix[0][1] = gOamMatrices[matrixNum].b;
            matrix[1][0] = gOamMatrices[matrixNum].c;
            matrix[1][1] = gOamMatrices[matrixNum].d;
        ...
        if (vcount >= (y - half_height) && vcount < (y + half_height))
        {
            ...
            for (int local_x = -half_width; local_x <= half_width; local_x++)
            {
                ...
                /* One-time pixel trace for affine debugging */
                if (isAffine) {
                    static int sPixDbg = 0;
                    if (sPixDbg == 0 && local_x == 0 && local_y == 0) {
                        printf("[PIX] mat=(%d,%d,%d,%d) lx=%d ly=%d -> tx=%d ty=%d w=%d h=%d hw=%d hh=%d\n",
                            matrix[0][0], matrix[0][1], matrix[1][0], matrix[1][1],
                            local_x, local_y, tex_x, tex_y, width, height, half_width, half_height);
                        fflush(stdout);
                        sPixDbg = 1;
                    }
                }
...
void DrawFrame(uint16_t *pixels)
{
...

    {
        ...
        if (sDrawDbg < 5 && gOamMatrices[0].a != 0x100) {
            printf("[DrawFrame] gOamMatrices[0]=%d,%d,%d,%d [1]=%d,%d,%d,%d addr=%p\n",
                gOamMatrices[0].a, gOamMatrices[0].b, gOamMatrices[0].c, gOamMatrices[0].d,
                gOamMatrices[1].a, gOamMatrices[1].b, gOamMatrices[1].c, gOamMatrices[1].d,
                (void*)gOamMatrices);
            fflush(stdout);
            sDrawDbg++;
        }
    ...
    for (i = 0; i < DISPLAY_HEIGHT; i++)
    {
        ...
        unsigned int blendMode = (REG_BLDCNT >> 6) & 3;
        uint16_t backdropColor = *(uint16_t *)PLTT | 0x8000;
