...
void CreateBoxMon(struct BoxPokemon *boxMon, u16 species, u8 level, u8 fixedIV, u8 hasFixedPersonality, u32 fixedPersonality, u8 otIdType, u32 fixedOtId)
{
    ...
    // Shiny rate override: server config, cheats, or shiny charm
#ifdef PLATFORM_SDL2
    {
        extern int ImGuiBridge_IsAlwaysShiny(void);
        extern int ImGuiBridge_IsShinyCharm(void);
        extern u16 NetClient_GetServerShinyRate(void);
        ...
        else
        {
            ...
            {
                u16 serverRate = NetClient_GetServerShinyRate();
                if (serverRate > 0 && serverRate < 8192)
                    extraRolls = (8192 / serverRate) - 1;
            }
            if (ImGuiBridge_IsShinyCharm())
            {
                extern bool8 CheckBagHasItem(u16 itemId, u16 count);
                if (CheckBagHasItem(ITEM_SHINY_CHARM, 1) && extraRolls < 8)
                    extraRolls = 8;
            }

            for (int r = 0; r < extraRolls && !forceShiny; r++)
            {
                u32 testPers = Random32();
                if (GET_SHINY_VALUE(value, testPers) < SHINY_ODDS)
                    forceShiny = true;
            }
        }
    ...
    if (species && species != SPECIES_EGG)
    {
        ...
        if (event == FRIENDSHIP_EVENT_LEAGUE_BATTLE)
        {
            ...
            if (!(SAFE_TRAINER(gTrainerBattleOpponent_A).trainerClass == TRAINER_CLASS_LEADER
                || SAFE_TRAINER(gTrainerBattleOpponent_A).trainerClass == TRAINER_CLASS_ELITE_FOUR
