        ...
        if (*src == PLACEHOLDER_BEGIN)
        {
            ...
            case B_TXT_TRAINER1_CLASS: // trainer class name
                if (gTrainerBattleOpponent_A == TRAINER_SECRET_BASE)
                    ...
                    toCpy = gTrainerClassNames[SAFE_TRAINER(gTrainerBattleOpponent_A).trainerClass];
                break;
            case B_TXT_TRAINER1_NAME: // trainer1 name
                ...
                {
                    if (SAFE_TRAINER(gTrainerBattleOpponent_A).trainerClass == TRAINER_CLASS_RIVAL_EARLY
                     || SAFE_TRAINER(gTrainerBattleOpponent_A).trainerClass == TRAINER_CLASS_RIVAL_LATE
                     || SAFE_TRAINER(gTrainerBattleOpponent_A).trainerClass == TRAINER_CLASS_CHAMPION)
                        toCpy = GetExpandedPlaceholder(PLACEHOLDER_ID_RIVAL);
                    else
                        toCpy = SAFE_TRAINER(gTrainerBattleOpponent_A).trainerName;
                }
                break;
