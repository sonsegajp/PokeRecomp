...
int ProcessPlayerFieldInput(struct FieldInput *input)
{
    ...
    if (input->tookStep)
    {
        ...
        {
            extern void PokeRadar_Step(void);
            PokeRadar_Step();
        }
    ...
    // Check for interaction with online player
    if (input->pressedAButton)
    {
        extern int NetClient_IsConnected(void);
        extern int NetPlayerSprites_GetFacingPlayerId(void);
        extern int NetPlayerSprites_IsPlayerBusy(int);
        extern void NetInteraction_ShowMenu(int playerId);
        if (NetClient_IsConnected())
        {
            int facingId = NetPlayerSprites_GetFacingPlayerId();
            if (facingId >= 0 && !NetPlayerSprites_IsPlayerBusy(facingId))
            {
                NetInteraction_ShowMenu(facingId);
                return TRUE;
            }
        }
    }
