...
#ifdef PORTABLE
static bool8 sSoundDisabled = FALSE;
#define M4A_CHECK if (sSoundDisabled) return
#define M4A_CHECK_VAL(v) if (sSoundDisabled) return (v)
#else
#define M4A_CHECK
#define M4A_CHECK_VAL(v)
#endif
...
void m4aSoundInit(void)
{
#ifdef PORTABLE
    {
        ...
        gSoundInfo.plynote = ply_note;
        gSoundInfo.CgbSound = CgbSound;
        gSoundInfo.CgbOscOff = CgbOscOff;
        gSoundInfo.MidiKeyToCgbFreq = MidiKeyToCgbFreq;
        ...
        gSoundInfo.maxChans = 12;
        gSoundInfo.masterVolume = 12;
        gSoundInfo.pcmSamplesPerVBlank = 701;
        gSoundInfo.pcmDmaPeriod = 1;
        gSoundInfo.pcmFreq = 42048;
        gSoundInfo.divFreq = (16777216 / gSoundInfo.pcmFreq + 1) >> 1;
...
#ifdef PORTABLE
    {
        extern float *SoundMainGetMixBuffer(void);
        extern int SoundMainGetSamplesPerFrame(void);
        s32 nSamples = SoundMainGetSamplesPerFrame();
        if (nSamples > 0)
        {
            float *mixBuf = SoundMainGetMixBuffer();
            cgb_audio_generate(nSamples);
            extern struct AudioCGB gb;

            for (s32 i = 0; i < nSamples; i++)
            {
                mixBuf[i * 2 + 0] += gb.outBuffer[i * 2 + 0];
                mixBuf[i * 2 + 1] += gb.outBuffer[i * 2 + 1];
            }

            Platform_QueueAudio(mixBuf, nSamples * 2 * sizeof(float));
        }
    }
#endif
}
...
#ifdef PORTABLE
    {
        extern bool8 AssetLoader_GetMusicOverridePath(u16 songId, char *outPath, int outSize);
        extern void Platform_PlayMusicOverride(const char *path);
        char overridePath[512];
        if (AssetLoader_GetMusicOverridePath(n, overridePath, sizeof(overridePath)))
        {
            Platform_PlayMusicOverride(overridePath);
            return;
        }
    }
#endif
...
#ifdef PORTABLE
    if (mplay->info->trackCount == 0 && mplay->unk_8 != 0)
        mplay->info->trackCount = mplay->unk_8;
#endif
...
void m4aSongNumStartOrChange(u16 n)
{
...
#ifdef PORTABLE
    {
        extern bool8 AssetLoader_GetMusicOverridePath(u16 songId, char *outPath, int outSize);
        extern void Platform_PlayMusicOverride(const char *path);
        extern int Platform_IsMusicOverridePlaying(void);
        char overridePath[512];
        if (AssetLoader_GetMusicOverridePath(n, overridePath, sizeof(overridePath)))
        {
            if (!Platform_IsMusicOverridePlaying())
                Platform_PlayMusicOverride(overridePath);
            return;
        }
    }
...
void m4aSongNumStop(u16 n)
{
    ...
    {
        extern int Platform_IsMusicOverridePlaying(void);
        extern void Platform_StopMusicOverride(void);
        if (Platform_IsMusicOverridePlaying())
            Platform_StopMusicOverride();
    }
#endif
...
#ifdef PORTABLE
    if (mplayInfo->trackCount == 0)
        mplayInfo->trackCount = 2;
#endif
    mplayInfo->ident++;
