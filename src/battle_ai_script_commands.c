...
void BattleAI_HandleItemUseBeforeAISetup(void)
{
        ...
        )
    {
        ...
        {
            if (SAFE_TRAINER(gTrainerBattleOpponent_A).items[i] != 0)
            {
                BATTLE_HISTORY->trainerItems[BATTLE_HISTORY->itemsNo] = SAFE_TRAINER(gTrainerBattleOpponent_A).items[i];
...
void BattleAI_SetupAIData(void)
{
    ...
    AI_THINKING_STRUCT->aiFlags = SAFE_TRAINER(gTrainerBattleOpponent_A).aiFlags;
}
