...
bool16 AddTextPrinter(struct TextPrinterTemplate *textSubPrinter, u8 speed, void (*callback)(struct TextPrinterTemplate *, u16))
{
    ...
    {
        extern float ImGuiBridge_GetTextSpeed(void);
        float ts = ImGuiBridge_GetTextSpeed();
        if (ts > 1.0f && speed != TEXT_SKIP_DRAW && speed != 0)
            sTempTextPrinter.textSpeed = (u8)(speed / ts);
        else
