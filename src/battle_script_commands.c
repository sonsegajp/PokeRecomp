...
static void Cmd_ppreduce(void)
{
    ...
    {
        extern int ImGuiBridge_IsInfinitePP(void);
        gProtectStructs[gBattlerAttacker].notFirstStrike = 1;

        if (!ImGuiBridge_IsInfinitePP())
        {
    ...
    case 1: // calculate experience points to redistribute
        {
            ...
            {
                extern int ImGuiBridge_GetExpMultiplier(void);
                int expMul = ImGuiBridge_GetExpMultiplier();
                if (expMul > 1)
                    calculatedExp *= expMul;
            }
...
static void Cmd_getmoneyreward(void)
{
    ...
    if (gBattleOutcome == B_OUTCOME_WON)
    {
        ...
        {
            switch (SAFE_TRAINER(gTrainerBattleOpponent_A).partyFlags)
            {
            case 0:
                {
                    const struct TrainerMonNoItemDefaultMoves *party1 = SAFE_TRAINER(gTrainerBattleOpponent_A).party.NoItemDefaultMoves;
                    
                    lastMonLevel = party1[SAFE_TRAINER(gTrainerBattleOpponent_A).partySize - 1].lvl;
                }
                break;
            case F_TRAINER_PARTY_CUSTOM_MOVESET:
                {
                    const struct TrainerMonNoItemCustomMoves *party2 = SAFE_TRAINER(gTrainerBattleOpponent_A).party.NoItemCustomMoves;
                    
                    lastMonLevel = party2[SAFE_TRAINER(gTrainerBattleOpponent_A).partySize - 1].lvl;
                }
                break;
            case F_TRAINER_PARTY_HELD_ITEM:
                {
                    const struct TrainerMonItemDefaultMoves *party3 = SAFE_TRAINER(gTrainerBattleOpponent_A).party.ItemDefaultMoves;
                    
                    lastMonLevel = party3[SAFE_TRAINER(gTrainerBattleOpponent_A).partySize - 1].lvl;
                }
                break;
            case (F_TRAINER_PARTY_CUSTOM_MOVESET | F_TRAINER_PARTY_HELD_ITEM):
                {
                    party4 = SAFE_TRAINER(gTrainerBattleOpponent_A).party.ItemCustomMoves;
                    
                    lastMonLevel = party4[SAFE_TRAINER(gTrainerBattleOpponent_A).partySize - 1].lvl;
                }
                break;
            }
            for (; gTrainerMoneyTable[i].classId != 0xFF; i++)
            {
                if (gTrainerMoneyTable[i].classId == SAFE_TRAINER(gTrainerBattleOpponent_A).trainerClass)
                    break;
            }
            party4 = SAFE_TRAINER(gTrainerBattleOpponent_A).party.ItemCustomMoves; // Needed to Match. Has no effect.
...
static void Cmd_handleballthrow(void)
{
    ...
    else
    {
        ...
        {
            extern int ImGuiBridge_IsCatchRate100(void);
            if (ImGuiBridge_IsCatchRate100())
                catchRate = 255;
        }
