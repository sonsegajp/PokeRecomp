...
{
#ifdef PORTABLE
    extern void NetClient_ResetOverworldFlag(void);
    NetClient_ResetOverworldFlag();
#endif
...
#ifdef PORTABLE
    {
        ...
        extern void NetClient_Poll(void);
        extern void NetClient_TickChat(void);
        if (NetClient_IsConnected())
        {
            NetClient_Poll();
            NetClient_TickChat();
            extern bool8 IsOverworldActive(void);
            if (!IsOverworldActive())
            {
                static int sBusySendCounter = 0;
                if (++sBusySendCounter >= 8)
                {
                    sBusySendCounter = 0;
    ...
    {
        extern void Platform_FlushSaveToServer(void);
        Platform_FlushSaveToServer();
    }
#endif
}
