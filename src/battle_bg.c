...
void DrawBattleEntryBackground(void)
{
    ...
    else
    {
        ...
        {
            u8 trainerClass = SAFE_TRAINER(gTrainerBattleOpponent_A).trainerClass;
            if (trainerClass == TRAINER_CLASS_LEADER)
            {
                LoadBattleTerrainEntryGfx(BATTLE_TERRAIN_BUILDING);
                return;
            }
            else if (trainerClass == TRAINER_CLASS_CHAMPION || trainerClass == TRAINER_CLASS_PKMN_PROF)
            {
                LoadBattleTerrainEntryGfx(BATTLE_TERRAIN_BUILDING);
                return;
            }
...
static u8 GetBattleTerrainOverride(void)
{
    ...
    {
        if (SAFE_TRAINER(gTrainerBattleOpponent_A).trainerClass == TRAINER_CLASS_LEADER)
            return BATTLE_TERRAIN_LEADER;
        else if (SAFE_TRAINER(gTrainerBattleOpponent_A).trainerClass == TRAINER_CLASS_CHAMPION
              || SAFE_TRAINER(gTrainerBattleOpponent_A).trainerClass == TRAINER_CLASS_PKMN_PROF)
