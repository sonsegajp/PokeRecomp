...
static void OpponentHandleDrawTrainerPic(void)
{
...
#ifdef PORTABLE
    extern int NetBattle_GetOpponentTrainerPic(void);
    int netPic = NetBattle_GetOpponentTrainerPic();
    if (netPic >= 0)
        trainerPicId = netPic;
    else
#endif
    if (gTrainerBattleOpponent_A == TRAINER_SECRET_BASE)
        ...
        trainerPicId = SAFE_TRAINER(gTrainerBattleOpponent_A).trainerPic;
...
#ifdef PORTABLE
    extern int NetBattle_GetOpponentTrainerPic(void);
    int netPic2 = NetBattle_GetOpponentTrainerPic();
    if (netPic2 >= 0)
        trainerPicId = netPic2;
    else
#endif
    if (gTrainerBattleOpponent_A == TRAINER_SECRET_BASE)
        trainerPicId = GetSecretBaseTrainerPicIndex();
