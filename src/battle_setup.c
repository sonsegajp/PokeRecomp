...
#ifdef PORTABLE
    {
        extern bool8 PokeRadar_IsActive(void);
        extern u16 PokeRadar_GetChainSpecies(void);
        extern void PokeRadar_BreakChain(void);
        if (PokeRadar_GetChainSpecies() != SPECIES_NONE)
        {
            u16 encSpecies = GetMonData(&gEnemyParty[0], MON_DATA_SPECIES);
            if (encSpecies != PokeRadar_GetChainSpecies())
                PokeRadar_BreakChain();
        }
    }
#endif
    // Announce catches in online chat
#ifdef PORTABLE
    if (gBattleResults.caughtMonSpecies != SPECIES_NONE)
    {
        extern int NetClient_IsConnected(void);
        extern void NetClient_SendChat(const char *);
        if (NetClient_IsConnected())
        {
            char catchMsg[128];
            u16 species = gBattleResults.caughtMonSpecies;
            u32 otId = GetMonData(&gEnemyParty[0], MON_DATA_OT_ID);
            u32 pid = GetMonData(&gEnemyParty[0], MON_DATA_PERSONALITY);
            bool8 isShiny = (((otId >> 16) ^ (otId & 0xFFFF) ^ (pid >> 16) ^ (pid & 0xFFFF)) < 8);
            GetSpeciesName(gStringVar1, species);
            char asciiName[16] = {0};
            for (int i = 0; i < 12 && gStringVar1[i] != 0xFF; i++)
            {
                u8 c = gStringVar1[i];
                if (c >= 0xBB && c <= 0xD4) asciiName[i] = 'A' + (c - 0xBB);
                else if (c >= 0xD5 && c <= 0xEE) asciiName[i] = 'a' + (c - 0xD5);
                else if (c == 0x00) asciiName[i] = ' ';
                else asciiName[i] = '?';
            }
            if (isShiny)
                snprintf(catchMsg, sizeof(catchMsg), "caught a SHINY %s!", asciiName);
            else
                snprintf(catchMsg, sizeof(catchMsg), "caught a %s!", asciiName);
            NetClient_SendChat(catchMsg);
        }
    }
#endif
...
static u8 GetTrainerBattleTransition(void)
{
    ...
    if (SAFE_TRAINER(gTrainerBattleOpponent_A).trainerClass == TRAINER_CLASS_ELITE_FOUR)
    {
    ...
    if (SAFE_TRAINER(gTrainerBattleOpponent_A).trainerClass == TRAINER_CLASS_CHAMPION)
        return B_TRANSITION_BLUE;
    if (SAFE_TRAINER(gTrainerBattleOpponent_A).trainerClass == TRAINER_CLASS_PKMN_PROF)
        return B_TRANSITION_OAK;
    if (SAFE_TRAINER(gTrainerBattleOpponent_A).doubleBattle == TRUE)
