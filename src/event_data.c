...
u16 *GetVarPointer(u16 idx)
{
    u16 *ptr;
#ifdef PORTABLE
    if (idx >= HOENN_VARS_BASE && idx < HOENN_VARS_BASE + HOENN_VARS_COUNT)
        return &gSaveBlock1Ptr->hoennVars[idx - HOENN_VARS_BASE];
#endif
    if (idx < VARS_START)
        return NULL;
...
u8 *GetFlagAddr(u16 idx)
{
    ...
    if (idx == 0)
        return NULL;
#ifdef PORTABLE
    if (idx >= HOENN_FLAGS_BASE && idx < HOENN_FLAGS_BASE + HOENN_FLAGS_COUNT)
    {
        u16 hoennIdx = idx - HOENN_FLAGS_BASE;
        return &gSaveBlock1Ptr->hoennFlags[hoennIdx / 8];
    }
#endif
