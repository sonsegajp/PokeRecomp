...
void LoadCompressedSpritePalette(const struct CompressedSpritePalette *src)
{
...
#ifdef PORTABLE
    {
        ...
        extern bool8 AssetLoader_LoadPokePalette(u16 species, void *dest, bool8 isShiny);

        if (AssetLoader_IsActive())
        {
            // Check if this is a Pokemon palette
            int normalIdx = (int)(src - gMonPaletteTable);
            int shinyIdx = (int)(src - gMonShinyPaletteTable);

            if (normalIdx >= 0 && normalIdx < NUM_SPECIES + 1)
            {
                if (AssetLoader_LoadPokePalette(normalIdx, gDecompressionBuffer, FALSE))
                {
            ...
            else if (shinyIdx >= 0 && shinyIdx < NUM_SPECIES + 1)
            {
                if (AssetLoader_LoadPokePalette(shinyIdx, gDecompressionBuffer, TRUE))
                {
                    dest.data = (void *)gDecompressionBuffer;
                    dest.tag = src->tag;
                    LoadSpritePalette(&dest);
...
{
#ifdef PORTABLE
    // For trainer pics (species == SPECIES_NONE), try loading from extracted assets
    if (species == SPECIES_NONE)
    {
        extern bool8 AssetLoader_LoadTrainerFrontPic(u16 id, void *dest);
        int idx = (int)(src - gTrainerFrontPicTable);
        if (idx >= 0 && idx < 800 && AssetLoader_LoadTrainerFrontPic(idx, buffer))
            return;
    }
#endif
...
#ifdef PORTABLE
    {
        extern bool8 AssetLoader_LoadPokePic(u16 species, void *dest, bool8 isFrontPic);
        if (AssetLoader_LoadPokePic(species, dest, isFrontPic))
        {
...
bool8 LoadCompressedSpritePaletteUsingHeap(const struct CompressedSpritePalette *src)
{
    ...
    {
        extern bool8 AssetLoader_IsActive(void);
        extern bool8 AssetLoader_LoadTrainerPalette(u16 id, void *dest);
        extern const struct CompressedSpritePalette gTrainerFrontPicPaletteTable[];

        if (AssetLoader_IsActive())
        {
            int trainerIdx = (int)(src - gTrainerFrontPicPaletteTable);
            if (trainerIdx >= 0 && trainerIdx < 200)
            {
                static u16 sTrainerPalBuf[16];
                if (AssetLoader_LoadTrainerPalette(trainerIdx, sTrainerPalBuf))
                {
                    dest.data = (void *)sTrainerPalBuf;
                    dest.tag = src->tag;
                    LoadSpritePalette(&dest);
                    return FALSE;
                }
