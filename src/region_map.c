...
#ifdef PORTABLE
static inline void SafeSetSpriteInvisible(struct Sprite *sprite, bool8 invisible)
{
    if (sprite != NULL)
        sprite->invisible = invisible;
}
#define SET_SPRITE_INVISIBLE(spr, val) SafeSetSpriteInvisible(spr, val)
#else
#define SET_SPRITE_INVISIBLE(spr, val) ((spr)->invisible = (val))
#endif
...
static void DisplayCurrentMapName(void)
{
    if (sRegionMap == NULL || sMapCursor == NULL)
        return;
...
static void SetMapEdgeInvisibility(u8 mapEdgeNum, bool8 invisible)
{
    ...
    if (mapEdgeNum == NELEMS(sMapOpenCloseAnim->mapEdges))
    {
        for (i = 0; i < NELEMS(sMapOpenCloseAnim->mapEdges); i++)
        {
            SET_SPRITE_INVISIBLE(sMapOpenCloseAnim->mapEdges[i]->sprite, invisible);
        }
    }
    else
    {
        SET_SPRITE_INVISIBLE(sMapOpenCloseAnim->mapEdges[mapEdgeNum]->sprite, invisible);
    }
}
...
{
    if (sMapOpenCloseAnim == NULL || sRegionMap == NULL)
        return;
...
{
    SET_SPRITE_INVISIBLE(sMapCursor->sprite, invisibile);
}
...
{
    SET_SPRITE_INVISIBLE(sPlayerIcon->sprite, invisible);
}
    ...
    if (iconNum == NELEMS(sMapIcons->flyIcons))
    {
        ...
        for (i = 0; i < NELEMS(sMapIcons->flyIcons); i++)
        {
            if (sMapIcons->flyIcons[i].region == whichMap || whichMap == 0xFF)
                SET_SPRITE_INVISIBLE(sMapIcons->flyIcons[i].sprite, invisible);
        }
    }
    else
    {
        if (sMapIcons->flyIcons[iconNum].region == whichMap)
            SET_SPRITE_INVISIBLE(sMapIcons->flyIcons[iconNum].sprite, invisible);
    }
}
    ...
    if (iconNum == NELEMS(sMapIcons->dungeonIcons))
    {
        ...
        for (i = 0; i < NELEMS(sMapIcons->dungeonIcons); i++)
        {
            if (sMapIcons->dungeonIcons[i].region == whichMap || whichMap == 0xFF)
                SET_SPRITE_INVISIBLE(sMapIcons->dungeonIcons[i].sprite, invisible);
        }
    }
    else
    {
        if (sMapIcons->dungeonIcons[iconNum].region != whichMap)
            SET_SPRITE_INVISIBLE(sMapIcons->dungeonIcons[iconNum].sprite, invisible);
    }
}
...
#ifdef PORTABLE
    {
        extern const u8 *Hoenn_GetMapsecName(u16 mapsec);
        const u8 *hoennName = Hoenn_GetMapsecName(mapsec);
        if (hoennName != NULL)
        {
            dst = StringCopy(dst0, hoennName);
            if (fill != 0)
            {
...
{
#ifdef PORTABLE
    // Hoenn fly destinations
    if (mapsec >= 208 && mapsec <= 223)
    {
        // Hoenn town/city MAPSECs -> warp to the outdoor map
        static const u8 sHoennFlyDest[][2] = {
            [208 - 208] = {43, 9},   // Littleroot
            [209 - 208] = {43, 10},  // Oldale
            [210 - 208] = {43, 0},   // Petalburg
            [211 - 208] = {43, 3},   // Rustboro
            [212 - 208] = {43, 11},  // Dewford
            [213 - 208] = {43, 1},   // Slateport
            [214 - 208] = {43, 2},   // Mauville
            [215 - 208] = {43, 14},  // Verdanturf
            [216 - 208] = {43, 13},  // Fallarbor
            [217 - 208] = {43, 12},  // Lavaridge
            [218 - 208] = {43, 4},   // Fortree
            [219 - 208] = {43, 5},   // Lilycove
            [220 - 208] = {43, 6},   // Mossdeep
            [221 - 208] = {43, 7},   // Sootopolis
            [222 - 208] = {43, 8},   // Ever Grande
            [223 - 208] = {43, 15},  // Pacifidlog
        };
        u8 idx = mapsec - 208;
        SetWarpDestinationToMapWarp(sHoennFlyDest[idx][0], sHoennFlyDest[idx][1], -1);
