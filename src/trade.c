...
static void CB2_CreateTradeMenu(void)
{
    ...
    case 10:
...
#ifdef PORTABLE
        {
            ...
            if (NetLink_IsActive())
                DrawTextWindowAndBufferTiles(gLinkPlayers[id].name, sMenuTextTileBuffers[GFXTAG_PLAYER_NAME_L], 0, 0, gDecompressionBuffer, 3);
            else
...
#ifdef PORTABLE
        extern bool8 NetLink_IsActive(void);
        if (NetLink_IsActive() || !gReceivedRemoteLinkPlayers)
#else
        if (!gReceivedRemoteLinkPlayers)
#endif
        {
